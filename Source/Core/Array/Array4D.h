...
#define INVALID -1
	...
	void Array4D<T, deviceType>::Resize(int nx, int ny, int nz)
	{
	...
	template<typename T, DeviceType deviceType>
	void Array4D<T, deviceType>::Reset()
	{
		switch (deviceType)
		{
		case CPU:
	...
	template<typename T, DeviceType deviceType>
	void Array4D<T, deviceType>::Release()
	{
		if (m_data != NULL)
		{
			switch (deviceType)
	...
	template<typename T, DeviceType deviceType>
	void Array4D<T, deviceType>::AllocMemory()
	{
		switch (deviceType)
		{
		case CPU:
	...
	template<typename T>
	using HostArray4D = Array4D<T, DeviceType::CPU>;

	template<typename T>
	using DeviceArray4D = Array4D<T, DeviceType::GPU>;

	//typedef DeviceArray4D<float> Grid1f;
	typedef DeviceArray4D<float4> Grid4f;
	//typedef DeviceArray4D<bool> Grid1b;
}
